...
| ~2025 | Metashape | 2.3 | `Metashape 2.3` | Latest published user manuals. Automatic colour enhancement. Texture-mode improvements (Keep UV + Natural texturing workaround via Texture Transfer); `out_of_focus_filter` parameter. `Sensor.axes` (`Aerial` / `Terrestrial`) added to the Python API for the per-sensor camera-axes convention. **Point releases:** 2.3.2 (build 22956) fixes the Python `==`/`None` interpreter abort ([forum 17579](https://www.agisoft.com/forum/index.php?topic=17579.0)) and the `camera.mask` → empty `chunk.mask_sets`/`masks` regression ([forum 17580](https://www.agisoft.com/forum/index.php?topic=17580.0)); **still open in 2.3.2**: the rig slave-sensor rotation-reference direction flip ([forum 17581](https://www.agisoft.com/forum/index.php?topic=17581.0)) and `.psz` silently dropping `sensor.axes` ([forum 17595](https://www.agisoft.com/forum/index.php?topic=17595.0)). See [*New features in Agisoft Metashape 2.3.x* (Agisoft KB)](https://agisoft.freshdesk.com/support/solutions/articles/31000177202). |
